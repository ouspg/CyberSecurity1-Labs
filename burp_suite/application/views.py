from flask import Blueprint, render_template, url_for, redirect, Response, current_app
...
def render_sitemap(pages):
    ...
    # inject placehodler flag
    xml.append('<url><loc>https://flag/{SITEMAP_FLAG}</loc></url>')
    for page in pages:
        xml.append("  <url><loc>%s</loc></url>" % page)
    xml.append('</urlset>')
    return "\n".join(xml)
